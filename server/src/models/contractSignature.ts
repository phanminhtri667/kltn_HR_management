...
type ContractSignatureCreation = Optional<ContractSignatureAttrs, 'id' | 'signer_employee_id' | 'signer_name' | 'signed_at' | 'signature_evidence'>;

module.exports = (sequelize: any) => {
  class ContractSignature extends Model<ContractSignatureAttrs, ContractSignatureCreation>
    ...
    static associate(models: any) {
      ContractSignature.belongsTo(models.EmploymentContract, { foreignKey: 'contract_id', as: 'contract' });
      ContractSignature.belongsTo(models.Employee, { foreignKey: 'signer_employee_id', targetKey: 'employee_id', as: 'signerEmployee' });
    }
  }
  ...
  ContractSignature.init(
    {
      ...
      signer_role: { type: DataTypes.ENUM('employee','hr','legal','manager','representative'), allowNull: false },
      sign_order: { type: DataTypes.INTEGER, allowNull: false, defaultValue: 1 },
      sign_status: { type: DataTypes.ENUM('pending','signed','rejected'), allowNull: false, defaultValue: 'pending' },
      signed_at: { type: DataTypes.DATE, allowNull: true },
      signature_evidence: { type: DataTypes.JSON, allowNull: true }
    },
    { sequelize, modelName: 'ContractSignature', tableName: 'contract_signatures', timestamps: false }
  );
